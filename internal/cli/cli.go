...
Usage:
  ...
  sentinel ao-lore evaluate [--baseline <json>] --observation <json> --out <json>
  sentinel version [--json]

	Commands: target baseline safety run compare monitor incident hold report watch triage security live-mutation issue-repair ao-lore version`)
}
